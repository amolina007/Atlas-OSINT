...
const feedUrl = (query: string, language: string, country: string) => {
  const languageMap: Record<string,string> = { es:"es-419", pt:"pt-BR", zh:"zh-CN" };
...
const parseFeed = (xml: string) => [...xml.matchAll(/<item>([\s\S]*?)<\/item>/gi)].map((match) => {
  const item = match[1];
...
export default async (req: Request) => {
  ...
  try {
    ...
    const unique = new Map<string, ReturnType<typeof parseFeed>[number]>();
